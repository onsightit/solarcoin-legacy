...
void RestartWallet(const char *parm, bool fOldParms)
{
    ...
    if (fNewVersion && !fBootstrapTurbo && !fRescan && !fEncrypt) // fNewVersion could be true while trying to do other restarts
    {
        ...
        command = QString(GetDataDir().string().c_str()) + QString("/") + QString(GetArg("-vFileName","solarcoin-setup.exe").c_str());
#else
#ifdef MAC_OSX
        ...
        newArgv.append(QString(GetDataDir().c_str()) + QString("/") + QString(GetArg("-vFileName","solarcoin-setup.pkg").c_str()));
#else
        // If Linux, just restart (already extracted solarcoin-qt from the zip in downloader.cpp).
        parm = NULL;
        fOldParms = false;
        newArgv.clear();
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("SolarCoin Version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  solarcoind [options]                     " + "\n" +
                  "  solarcoind [options] <command> [params]  " + _("Send command to -server or solarcoind") + "\n" +
                  "  solarcoind [options] help                " + _("List commands") + "\n" +
                  "  solarcoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "solarcoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("SolarCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("SolarCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file path (default: <cwd>/solarcoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: solarcoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 18188 or testnet: 18189)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 18181 or testnet: 18182)") + "\n" +
        ...
        "  -blockprioritysize=<n> "   + _("Set maximum size of high-priority/low-fee transactions in bytes (default: 17000)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  SolarCoin is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "SolarCoin server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("SolarCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of SolarCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart SolarCoin to complete") << "\n";
