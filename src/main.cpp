...
#include <inttypes.h>    // Check for negative or overflow output values
...
unsigned int ComputeMinStake(unsigned int nBase, int64_t nTime, unsigned int nBlockTime)
{
    ...
    bnResult.SetCompact(nBase);
    bnResult *= 2;
    while (nTime > 0 && bnResult < bnProofOfStakeLimit)
    {
        // Maximum 200% adjustment per day...
        bnResult *= 2;
        nTime -= 24 * 60 * 60;
    }
    if (bnResult > bnProofOfStakeLimit)
        bnResult = bnProofOfStakeLimit;
    return bnResult.GetCompact();
}
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                // Verify signature
                if (nVersion == CTransaction::CURRENT_VERSION) // Different signature rules for PoST
                {
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload)
    {
        ...
        if (nUpgraded > 0)
            printf("SetBestChain: %d of last 100 blocks above version %d\n", nUpgraded, CBlockHeader::CURRENT_VERSION);
        if (nUpgraded > 100/2)
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && nVersion == CBlockHeader::CURRENT_VERSION)
        return DoS(100, error("AcceptBlock() : reject proof-of-work at height %d", nHeight));

    if (IsProofOfStake() && nVersion < CBlockHeader::CURRENT_VERSION)
    ...
    if (CheckpointsMode == Checkpoints::CPSTRICT && !cpSatisfies)
        return error("AcceptBlock() : rejected by synchronized checkpoint");

    if (CheckpointsMode == Checkpoints::CPADVISORY && !cpSatisfies)
