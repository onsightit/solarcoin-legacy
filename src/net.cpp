...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SolarCoin Version " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
static const char *strDNSSeed[][2] = {
    {"dnsseed1", "dnsseed.solarcoin.org"},
    {"dnsseed2", "dnsseed2.solarcoin.org"},
    {"dnsseed3", "dnsseed3.solarcoin.org"},
...
void ThreadDNSAddressSeed(void* parg)
{
    ...
    RenameThread("solarcoin-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("solarcoin-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("solarcoin-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("solarcoin-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SolarCoin is probably already running."), addrBind.ToString().c_str());
        else
