...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("netmhashps",     GetPoWMHashPS(pindexBest)));
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "SolarCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "SolarCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SolarCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SolarCoin is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (nBestHeight >= FORK_HEIGHT_1)
        ...
        result.push_back(Pair("sizelimit", (int64_t)MAX_BLOCK_SIZE_1M));
